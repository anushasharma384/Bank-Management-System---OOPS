...
public class Main {
    ...
    public static boolean mainMenu() {
        ...
        switch (res) {
            case 1: customer();
                    break;
            case 2: getAllAccounts();
                    break;
            case 3: getAllLoans();
                    break;
            case 4: getTotalMoney();
                    break;
            case 5: return false;
            default:
    ...
    public static void customer() {
        while (true) {
            ...
            switch (res) {
                case 1: addCustomer();
                    break;
                case 2: getListOfCustomers();
                    break;
                case 3: selectCustomer();
                    break;
                case 4: return;
                default:
                    System.out.println("Invalid choice !!");
    ...
    public static void getListOfCustomers() {
        ...
        for(int i=0; i<customerList.size(); i++){
            ...
            System.out.println((i+1) + ". " + c.showCustomerName() + " - " + (c.getCustomerType().equals("vip") ? "VIP" : "REGULAR"));
        }
    }
    ...
    public static void selectCustomer() {
        ...
        while (true){
            ...
            if(res == 1){
                ...
                int temp = sc.nextInt();
                Account acc;
                if(temp == 1) account(c.getAccountDetails("saving"));
                else if(temp == 2) account(c.getAccountDetails("current"));
                ...
                int temp = sc.nextInt();
                if(temp == 1) c.createSavingAccount();
                else if(temp == 2) c.createCurrentAccount();
                else System.out.println("Invalid choice...");
            } else if(res == 3){
    ...
    public static void getAllAccounts() {
        ...
        for(Customer c: customerList) {
            if(c.saving != null) allAccounts.add(c.saving);
            if(c.current != null) allAccounts.add(c.current);
        }
    ...
    public static void getTotalMoney() {
        ...
        List<Customer> customerList = b.getCustomerList();
        for(Customer c: customerList) {
            if(c.saving != null) totalMoney += c.saving.checkBalance();
            if(c.current != null) totalMoney += c.current.checkBalance();
        }
